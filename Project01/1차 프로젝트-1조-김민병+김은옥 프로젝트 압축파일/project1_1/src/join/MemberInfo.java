...
public class MemberInfo extends JFrame {
	...
	public MemberInfo(String id, String pw) throws Exception {
		...
		Reservation.ReservationDAO dao_r = new Reservation.ReservationDAO(); // 사용자의 예매 내역을 가져오기 위해서 reservationDAO 연결

		// 가방을 가져옴
		MembershipVO bag = dao.one(id);
		ArrayList<ReservationVO> list = dao_r.call(id); // 전제 목록을 받아오는 것이기 때문에 arraylist를 사용하여 값을 저장해준다

		// 테이블안에 생성될 값
		Object rowData[][] = new Object[list.size()][]; // 목록의 수와 값으로 정보를 받기 때문에 이차원배열을 사용하서 값을 불러와야한다.
		for (int i = 0; i < list.size(); i++) { // 리스트에 저장된 목록의 수 만큼 반복문을 돌려준다.
			ReservationVO bag_r = list.get(i);
			Object[] o = { bag_r.getReserv_num(), bag_r.getSubject(), bag_r.getRegion(), bag_r.getDate(),
					bag_r.getRoom_num(), bag_r.getTime(), bag_r.getSeat_num() }; // 이차원배열줄 값을 지정해주지 않은 배열에 들어갈 값들을 지정해주고 있다
			rowData[i] = o;
		}
		Object colNames[] = { "예매번호", "영화제목", "영화관지점", "날짜", "상영관번호", "상영시간", "좌석번호" }; // 목록의 제목부분을 지정해주었다.
		...
		cancle.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				JOptionPane.showMessageDialog(null, "예매가 취소되었습니다."); // 호출된 목록에서 예매취소할 번호의 값을 입력 받고 그 목록에 한에서 예매 취소를 진행하기 위해서
